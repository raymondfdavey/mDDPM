...
if [ "$#" -ne 5 ]; then
    echo "PROBLEM"
    echo "Usage: $0 <ORIGINAL_DATA_DIR> <PROCESSED_DATA_DIR> <FINAL_DATA_TARGET_DIR> <SPLITS_DATA_DIR> <COMPUTE_RESOURCE>"
    exit 1
fi
original_data_dir="$1"
...
bash prepare_IXI.sh "$original_data_dir/IXI" "$processed_data_dir" "$resource"
echo "IXI PREPROCESSING...DONE!"

# Run prepare_MSLUB.sh
echo "RUNNING MSLUB PREPROCESSING"
bash prepare_MSLUB.sh "$original_data_dir/MSLUB" "$processed_data_dir" "$resource"
